...
-- RLS Policy: Simple permission model
DROP POLICY IF EXISTS "Users can view packing protocols for their company" ON public.packing_protocols;
CREATE POLICY "Users can view packing protocols" 
  ON public.packing_protocols 
  FOR SELECT 
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users can create packing protocols for their company" ON public.packing_protocols;
CREATE POLICY "Users can create packing protocols" 
  ON public.packing_protocols 
  FOR INSERT 
  WITH CHECK (auth.uid() IS NOT NULL AND created_by = auth.uid());

DROP POLICY IF EXISTS "Users can update packing protocols for their company" ON public.packing_protocols;
CREATE POLICY "Users can update packing protocols" 
  ON public.packing_protocols 
  FOR UPDATE 
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users can delete packing protocols for their company" ON public.packing_protocols;
